...
export const BookingForm = ({ booking, handleClick }: IBookingProps) => {
    ...
    const checkAvailability = (date?: string, numberOfGuests?: number) => {
        ...
        if (date) {
            ...
            if (numberOfGuests) {
                isAvailable = getTotalPersonsForDateAndTime(newBooking.date, newBooking.time) + (numberOfGuests ?? 0) > totalAvailableSeats;
        ...
        console.log("inne i func", [{isAvailableNumberOfGuests: isAvailable}, availability])
    ...
    const updateBookingField = (key: keyof Booking, value: string | number) => {
        ...
        if (key === "time" && sittingAvailability && !sittingAvailability[value as string]) { 
            setValidInputStyle(prevState => ({ ...prevState, [key]: "invalid" }));
            return;
        }
    ...
    const renderInputField = (key: string, value: string | number | Customer, inputType: string) => {
        const min = key === "date" ? getTodayString() : key === "numberOfGuests" ? 1 : undefined;
        return (
            <input
                type={inputType}
                id={key}
                ...
                {Object.keys(newBooking).map((key, index) => {
                    ...
                    if (key === "customer") {
                        return Object.keys(newBooking.customer).map((customerKey, customerIndex) => (
                            <div key={`customer-${customerIndex}`} className="container">
                                <input
                                    type="text"
                                    placeholder={customerKey.charAt(0).toUpperCase() + customerKey.slice(1)}
                                    id={`customer-${customerKey}`}
                                    value={newBooking.customer[customerKey as keyof Customer] || ''}
                                    className={`${validInputStyle[customerKey] ? validInputStyle[customerKey] : ''} form__input`}
                                    onChange={(e) => updateCustomerField(customerKey as keyof Customer, e.target.value)}
                                    required={true}
                                />
                                 <label htmlFor={`customer-${customerKey}`} className="form__label">{customerKey.charAt(0).toUpperCase() + customerKey.slice(1)}</label>
                            </div>
                        ));
                    }
                    let inputField;
                    ...
                    return (
                        <div className="container" key={index}>
                            {inputField}
                            <label htmlFor={key} className="form__label">{key.charAt(0).toUpperCase() + key.slice(1)}</label>
                        </div>
                    )
                })}
